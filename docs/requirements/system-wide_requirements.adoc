...
** Zur Überprüfung wird versucht das Verwaltungsfrontend außer des HTW-Netzes zu öffnen sowie sich anzumelden mit falschen Login-Daten.
...
** Zur Überprüfung werden PC-Monitore verschiedenster Größen bedient.
...
** Zur Überprüfung wird an einem Rechner, im Verwaltungsfrontend, ein bestehender Datensatz modifiziert, während parallel an einem anderem Rechner, im Informationsfrontend, dieser angesehen.
* NFAR-3: Das System muss paralleles Lesen und Schreiben im Verwaltungsfrontend unterstützen. (Multi-User-System)
** Zur Überprüfung werden jewerils zwei verschiedene Login-sessions laufen, um einerseits einen Datensatz zu lesen und andererseits parallel dazu einen Datensatz zu schreiben.
...
* NFAP-2: Datenbankrelevante Funktionen sollten möglichst schnell, maximal 2 Sekunden, ausführbar sein.
...
* strikte Kompetenzvergabe an spezifische Rollen

=== Organisatorische Randbedingungen
//Angaben ergänzen, nicht relevante Unterpunkte streichen oder auskommentieren
* ausführliche und aussagekräftige Dokumentation
* verständliches Näherbringen an die Stabstelle
