...
# get all institutes saved in tbl__institutes
def institutes_ret():
    '''
    Alle Institute, die in tbl__institutes gespeichert sind, holen
    '''
...
def ag_type_ret():
    ...
    cnxn = Login.newConnection()
    cur = cnxn.cursor()
    cur.execute('SELECT ID, deu FROM tbl__partnership_type ORDER BY deu')
    x = cur.fetchall()
    payload = []
    for i in x:
        content = {
...
def all_countries():
    '''
    Alle Länder, die in tbl__country gespeichert sind, für die Filterwerte holen
    '''
    cnxn = Login.newConnection()
    cur = cnxn.cursor()
    #cur.execute("SELECT de, id FROM tbl__country ORDER BY de")
    #cur.execute("CREATE VIEW val_coun AS SELECT de, id FROM tbl__country WHERE id in (SELECT id FROM auslandsamt.country) ORDER BY de ")
    #cur.execute("SELECT de, id FROM val_coun")
    cur.execute("SELECT de, id FROM tbl__country WHERE id in (SELECT id FROM auslandsamt.country) ORDER BY de ")
    x = cur.fetchall()
    payload = []
    for i in x:
        content = {
...
def faculty():
    '''
    Alle Fakultäten, die in tbl__faculty gespeichert sind, holen
    '''
    cnxn = Login.newConnection()
    cur = cnxn.cursor()
    cur.execute("SELECT ID, deu FROM tbl__faculty ORDER BY deu")
    x = cur.fetchall()
    payload = []
    for i in x:
        content = {
...
def new_mentor(columns, values):
    ...
    query = "INSERT INTO tbl__mentor (" + query_parameter[0] + ") VALUES(" + query_parameter[1] + ")"
    try:
        insert = tuple(values)
        print(insert)
...
def new_object(object_type, tuple_columns, tuple_values, inst_name=None, inst_ps_type=None):
    ...
    type_dict = {
        'mentor': 'tbl__mentor',
        'institute': 'tbl__institute',
        'agreement': 'tbl__mobility_agreement',
        'restriction': 'tbl__mobility_agreement_x_course',
        'faculty': 'tbl__faculty',  
        'country': 'tbl__country'      
    } 
    ...
    try:
        # insert into tbl__institute
        print(query, parameters)
        cur.execute(query, parameters,)
        cnxn.commit()
        # query for insert into tbl__partnership
...
def return_countries():
    ...
    cnxn = Login.newConnection()
    cur = cnxn.cursor()
    #cur.execute('SELECT de, en, erasmus FROM tbl__country ORDER BY de')
    cur.execute("SELECT de, en, erasmus FROM tbl__country WHERE id in (SELECT id FROM auslandsamt.country) ORDER BY de ")
...
def return_courses():
    ...
    cnxn = Login.newConnection()
    cur = cnxn.cursor()
    cur.execute('SELECT deu, eng, ID FROM tbl__course ORDER BY deu')
    x = cur.fetchall()
    payload = []
    for row in x:
        content = {
...
def return_mentor():  # get all mentor information and store on client storage
    ...
    query = """SELECT m.ID, m.faculty_ID, m.active, m.gender_ID, m.title, m.firstname, m.lastname, count(mentor_ID) 
                FROM tbl__mobility_agreement ma 
                RIGHT JOIN tbl__mentor m  
...
def return_faculties():
    ...
    query = """SELECT f.deu, f.eng, COUNT(m.faculty_ID), f.id
                FROM tbl__mobility_agreement m
                JOIN tbl__faculty f 
...
def edit(keys, values, change_id, change_type):  # institute = institute ID
    ...
    '''
    tbl_names = {'institute': "tbl__institute", 'agreement': "tbl__mobility_agreement", 'restriction': "tbl__mobility_agreement_x_course", 'mentor': 'tbl__mentor'}
...
def checkLength(key, object_id):
    ...
    cur = cnxn.cursor()
    dict = {
        'institute': ['agreements_for_institute', 'tbl__institute'],
        'agreement': ['courses_for_agreement', 'tbl__mobility_agreement']
    }
    if key in dict:
        ...
        delete('tbl__mobility_agreement_x_course', strip)
...
def delete(tbl, row_id):
    ...
    cnxn = Login.newConnection()
    cur = cnxn.cursor()
    if tbl == 'tbl__institute':
        query = f"DELETE FROM tbl__partnership WHERE institute_ID = {row_id}"
        cur.execute(query)
        cnxn.commit()
...
def facultyReport(faculty_id):
    ...
    cur.execute(f"SELECT c.de, i.eng, pt.deu, CONCAT(m.title, ' ',m.firstname, ' ', m.lastname) AS mentordata, ma.until_date \
			FROM tbl__mobility_agreement AS ma, \
				tbl__mentor AS m, \
				tbl__partnership_type AS pt, \
				tbl__country AS c, \
				tbl__partnership AS p, \
                tbl__institute AS i \
            WHERE \
                (\
				((ma.mentor_ID = m.ID) \
...
def erasmusReport():
    ...
    cur.execute("""SELECT c.de, i.eng, CONCAT(m.title, ' ',m.firstname, ' ', m.lastname) AS mentordata, ma.until_date, pt.deu
                FROM tbl__mobility_agreement AS ma
                JOIN tbl__partnership p ON p.ID = ma.partnership_ID
                JOIN tbl__partnership_type pt ON pt.ID = p.partnership_type_ID
                JOIN tbl__institute i ON i.ID = p.institute_ID
                JOIN tbl__country c ON c.ID = i.country_ID
                JOIN tbl__mentor m ON m.ID = ma.mentor_ID
